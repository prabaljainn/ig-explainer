...
# espeak-ng keeps its data path in a fixed ~160-char buffer; a deeper checkout makes Kokoro fail with a
# confusing "/Users/runner/.../phontab: No such file or directory" from the vendored library.
DATA_LEN=${#ROOT}
if [ "$DATA_LEN" -gt 90 ]; then
  opt "checkout path is $DATA_LEN characters deep" "espeak-ng truncates its data path near 160 including the venv suffix; if narration fails with a phontab error, move the checkout somewhere shorter (e.g. ~/ig-explainer)"
fi
